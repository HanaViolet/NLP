...
### 1. 论文数学纠错：Attention Masking 的因果性 (Week 10-11)

**项目背景**：在完成课程作业时，我详细研读了相关学术论文关于 Segment-Based Attention Masking 的内容。

**核心发现**：在阅读过程中，我发现原论文对因果掩码（Causal Masking）的定义存在问题。原始定义将掩码矩阵定义为上三角形式，这意味着当前 Token 会被允许关注到未来的 Token，严重违反了 GPT 的自回归属性（Autoregressive Property）。

**纠错过程**：我推导了正确的**下三角掩码**定义，确保每个位置只能关注自己及之前的 Token，维持因果的、自回归的生成过程。具体推导如下：

**错误定义分析**：
上三角矩阵允许过度关注，破坏因果性

**正确的因果掩码**：
通过下三角矩阵（掩码矩阵 M，其中 M[i,j] = 1 if i >= j else -∞），确保自回归特性。应用于注意力时，M[i,j] = -∞ (i < j) 的位置会在 softmax 后产生近似 0 的权重。

**成果**：提交了详细的数学勘误报告，清晰展示了因果性约束对生成任务的重要性。
