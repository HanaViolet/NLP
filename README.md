...
**第二张图 - 我的纠正分析**：
这部分展示了我发现的问题及正确定义。通过下三角掩码，确保严格的因果约束。
...
**![swimvg](photos/train_metrics.png )** — 在此处可插入 SwimVG 模型在自有数据集上的训练指标、准确率对比图表、损失曲线或模型可视化结果
...
**![nanogpt](photos/para.png)** — 在此处可插入 nanoGPT 训练日志截图，显示"number of parameters: 10.65M"的输出信息
...
**![loss](photos/loss_curve.png)** — 在此处可插入 nanoGPT 的训练损失曲线，展示从初始 Loss ~4.28 逐步收敛到 ~0.87 的过程（超过 5000 个迭代步）
